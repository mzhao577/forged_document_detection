...
Usage:
    python generate_charts_HTML_batch.py <input_path> <output_dir> <N> [createPDF]
    ...
    createPDF:   True or False — whether to convert HTML to PDF (default: False)
"""
...
def generate_chart(input_file: str, output_dir: str, variation: int, total: int, create_pdf: bool = False):
...
def process_file(input_file: str, output_dir: str, n: int, create_pdf: bool = False):
    """Generate N variations for a single prompt file."""
    for v in range(1, n + 1):
        generate_chart(input_file, output_dir, variation=v, total=n, create_pdf=create_pdf)


def main():
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print("Usage: python generate_charts_HTML_batch.py <input_path> <output_dir> <N> [createPDF]")
        sys.exit(1)
    ...
    if len(sys.argv) == 5:
        if sys.argv[4].lower() == "true":
            ...
            print("Error: createPDF must be 'True' or 'False'.")
            sys.exit(1)

    if os.path.isfile(input_path):
        print(f"Processing '{input_path}' — generating {n} variation(s)... (createPDF={create_pdf})")
        process_file(input_path, output_dir, n, create_pdf=create_pdf)
        ...
        for i, fname in enumerate(files, 1):
            ...
            process_file(os.path.join(input_path, fname), output_dir, n, create_pdf=create_pdf)
